---
title: "Fail builds on high risk"
sidebarTitle: "Fail on high risk"
description: "Block builds on any critical or high-severity finding, layered on top of the AIVSS score gate."
icon: "circle-x"
---

The default `--fail-under` gate is a *score* gate — it blocks when the
...
If your only concern is the aggregate AIVSS, the score gate from [GitHub Actions](/ci-cd/github-actions) (or [GitLab CI](/ci-cd/gitlab-ci)) is enough on its own — skip this page.
...
<CardGroup cols={2}>
  ...
  <Card title="Reports" icon="file-text" href="/reports/overview">
    The structure of `scan.json` — including the per-finding facets
    this gate parses.
  </Card>
  <Card title="AIVSS score" icon="chart-line" href="/reports/aivss-score">
    The five-step formula + the `mode_authoritative` rule that gates
    `--fail-under`.
  </Card>
</CardGroup>
